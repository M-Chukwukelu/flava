...
// If the user has already liked the post, remove the like. If not, add the like.
// Also send a nitification to the post owner if the liker is not the post owner.
export const likeUnlikePost = async (req, res) => {
	try {
		...
		if (userLikedPost) {
			...
			// I should not get a notification if I like my own post
			if (post.user.toString() !== req.user._id.toString()) {
				...
				// Proceed with saving or sending the notification as needed
...
export const getAllPosts = async (req, res) => {
	try {
		...
		if (posts.length === 0) {
			return res.status(200).json([]); // User has no posts yet.
		}
...
export const getLikedPosts = async (req, res) => {
	...
	try {
		...
		const likedPosts = await Post.find({ _id: { $in: user.likedPosts } }) // Get all posts that the user has liked
			.populate({
				path: "user",
				select: "-password",
			});
